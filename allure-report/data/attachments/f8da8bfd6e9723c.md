...
```ts
  ...
  64 |         console.log(`path.resolve(__dirname, "../data", ${this.testInfo.file.replace(/\.spec\.(js|ts)$/, ".json")})`);
  65 |         const filePath = path.resolve(__dirname, "../data", this.testInfo.file.replace(/\.spec\.(js|ts)$/, ".json"));
  66 |         try {
  67 |             const raw = fs.readFileSync(filePath, "utf-8");
  68 |             console.log("*********************************");
  69 |             console.log(`Loaded test data from: ${filePath}`);
  70 |             console.log(JSON.parse(raw));
  71 |             return JSON.parse(raw);
  72 |         } catch (err) {
> 73 |             throw new Error(`Failed to load test data from "${filePath}": ${err.message}`);
     |                   ^ Error: Failed to load test data from "/Users/thangaraj.r/Documents/playwright-pom-framework/tests/specs/excellencelabtest1.json": ENOENT: no such file or directory, open '/Users/thangaraj.r/Documents/playwright-pom-framework/tests/specs/excellencelabtest1.json'
  74 |         }
  75 |     }
  76 | 
  77 |     async getSection(section, fileName) {
  78 |     const data = await this.loadTestData(fileName);
  79 |     if (!data[section]) {
  80 |         throw new Error(`Section "${section}" not found in ${fileName}`);
  81 |     }
  82 |     return data[section];
  83 | }
  84 | 
  85 |     async loadDataForSpec() {
  86 |     const specBase = path.basename(this.testInfo.file);          
  87 |     const jsonName = specBase.replace(/\.spec\.(js|ts)$/, ".json"); 
  88 |     return await this.loadTestData(jsonName);
  89 | }
  90 | 
  91 | 
  92 | 
