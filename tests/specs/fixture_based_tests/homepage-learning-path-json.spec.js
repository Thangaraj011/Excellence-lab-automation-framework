...
test.describe.serial("Homepage Learning path status update validation", () => {
  ...
  test(
    "Learning path specific individual content status update to Inprogress validation",
    { tag: ["@LPtests"] },
    async ({ authPage, homePage, learningRecordsPage, genericUtils }) => {
      const individualContentName = await genericUtils.getValueByKey(
        "IP_IndividualContentName",
      );
      const expectedStatus =
        await genericUtils.getValueByKey("IP_ContentStatus");
      ...
      await test.step("Mark content as In Progress", async () => {
        const progressButton = await homePage.getContentProgressButton(
      ...
      await learningRecordsPage.verifyInLearningRecords(
        individualContentName,
        todaysDate,
        expectedStatus,
      );
  ...
  test(
    "Learning path specific individual content status update to Completed validation",
    { tag: ["@LPtests"] },
    async ({ authPage, homePage, learningRecordsPage, genericUtils }) => {
      const individualContentName = await genericUtils.getValueByKey(
        "IP_IndividualContentName",
      ...
      const currentProgress =
        await homePage.getLearningPathProgressPercentage();
  ...
  test(
    "Entire Learning path status update to Completed validation",
    { tag: ["@LPtests"] },
    async ({ authPage, homePage, learningRecordsPage, genericUtils }) => {
      const currentProgress =
        await homePage.getLearningPathProgressPercentage();
      ...
      await learningRecordsPage.verifyStatusOfTheContents(
        contentNames,
        todaysDate,
      );
    },
