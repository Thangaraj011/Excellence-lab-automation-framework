...
```
TypeError: rowElement.locator is not a function
```
...
```ts
  ...
  35 |         const rowElement = await learningRecordsPage.findTitlesAcrossPages(data.contentTitle, data.contentSource);
> 36 |         await rowElement.locator('button.ant-dropdown-trigger').click()
     |                          ^ TypeError: rowElement.locator is not a function
  37 |         await learningRecordsPage.editOption.click();
  38 |         await authPage.pause();
  39 |     });
  40 | 
  41 | 
  42 | })
