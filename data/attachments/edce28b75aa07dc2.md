...
```
TypeError: learningRecordsData.findTitlesAcrossPages is not a function
```
...
```ts
  ...
  13 |     })
  14 | 
  15 |     test.only('Single Individual content learning recods details validation', {tag: "@learningRecords"}, async({authPage, homePage, learningRecordsPage, genericUtils}) =>{
> 16 |         await learningRecordsData.findTitlesAcrossPages(data.contentTitles);
     |                                   ^ TypeError: learningRecordsData.findTitlesAcrossPages is not a function
  17 |     });
  18 | })
```