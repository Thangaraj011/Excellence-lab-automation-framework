...
```ts
  42  |       await homePage.selectDueDates(data.dueDateStart, data.dueDateEnd);
  ...
  140 |       const apiContentTitlesRetrieved = await homePage.getAllTitlesFromResponses(managerResponse,adminResponse);
  141 |       const uiContentTitlesDisplayed = await homePage.getContentNames();
> 142 |       expect(apiContentTitlesRetrieved).toEqual(uiContentTitlesDisplayed);
      |                                         ^ Error: expect(received).toEqual(expected) // deep equality
  143 |     },
  144 |   );
  145 | });
