...
```
TypeError: element.locator is not a function
```
...
```ts
  ...
  35 |         const element = await learningRecordsPage.findTitlesAcrossPages(data.contentTitle, data.contentSource);
> 36 |         await element.locator('/td/button').click()
     |                       ^ TypeError: element.locator is not a function
