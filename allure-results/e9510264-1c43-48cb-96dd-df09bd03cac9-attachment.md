...
Call log:
  - waiting for getByRole('combobox', { name: 'Sort courses' }).first().locator('.ant-select-item-option-content').filter({ hasText: 'asc' })
...
```ts
  ...
  332 |     await this.managerSortByDropdown
  333 |       .locator(".ant-select-item-option-content")
